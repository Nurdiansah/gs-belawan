...
if (isset($_POST['approve'])) {
    ...
    // Update approval SO
    $query = "UPDATE so SET status = '2', app_mgr_ga = NOW()
                WHERE id_so ='$id' ";
    ...
    while ($dataUser = mysqli_fetch_assoc($queryUser)) {
        $link = "url=index.php?p=verifikasi_sr&lvl=kordinator_pajak";
        $name = $dataUser['nama'];
        $email = $dataUser['email'];
        $subject = "Verifikasi Service Order " . $dataEmail['nm_barang'];
