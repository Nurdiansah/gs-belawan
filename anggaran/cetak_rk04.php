...
if (isset($_GET['tahun']) && isset($_GET['project'])) {
    ...
    $sub_divisi = dekripRambo($_GET['sub_divisi']);
}
...
// ngambil data divisi
$dataDivisi = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM parent_divisi WHERE id_parent = '$divisi'"));
...
if (!isset($_SESSION['username_blw']) || $_SESSION['level_blw'] != "anggaran") {
    ...
    // header("Content-Disposition: attachment; filename=RK04-" . $tahun . "-" . $dataPT['nm_pt'] . "-" . $dataDivisi['nm_parent'] . "-" . $dataSubDivisi['nm_divisi'] . ".xls");
    // header("Content-Disposition: attachment; filename=RK04-" . $tahun . ".xls");
...
?>
    ...
    <table>
        ...
        <tr>
            <td colspan="4"><b>Divisi - <?= $dataDivisi['nm_parent']; ?></b></td>
        </tr>
        <tr>
            <td colspan="4"><b>Sub Divisi - <?= $dataSubDivisi['nm_divisi']; ?></b></td>
        </tr>
        <tr>
            <td></td>
        </tr>
