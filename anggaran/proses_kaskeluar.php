<?php  
    ...
    if (isset($_GET['aksi']) && isset($_GET['id'])) {
        ...
        if ($_GET['aksi'] == 'edit') {
            header("location:?p=detail_proseskaskeluar&id=$id");
        } else if ($_GET['aksi'] == 'hapus') {
            header("location:?p=hapus_joborder&id=$id");
        } 
    }

        $query = mysqli_query($koneksi, "SELECT * FROM bkk WHERE status_bkk='3' ORDER BY tgl_bkk DESC  ");    
                				...
                				<tr>
                					<?php 
                					 ...
                					 ?>
                						...
                						<td>
                                            <a href="?p=proses_kaskeluar&aksi=edit&id=<?= $row['id_bkk']; ?>"><span data-placement='top' data-toggle='tooltip' title='Lihat'><button  class="btn btn-info">Lihat</button></span></a>                     
                                        </td>              					
                				</tr>
                            <?php 
                            $no++; endwhile; } ?>
